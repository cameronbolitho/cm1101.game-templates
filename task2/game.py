...
def print_room_items(room):
    ...
    if items != []:
        print(f"There is {list_of_items(items)} here.")
        print()
...
def print_menu(exits, room_items, inv_items):
    ...
    for item in room_items:
        ...
        item_name = item["name"]
        print(f"TAKE {item_id} to take {item_name}.")

    # Iterate over inventory items
    for item in inv_items:
        item_id = item["id"].upper()
        item_name = item["name"]
        print(f"DROP {item_id} to take {item_name}.")
...
def execute_take(item_id):
    ...
    """
    # Calculate the amount of additional stuff the player can carry
    free_mass = 3
